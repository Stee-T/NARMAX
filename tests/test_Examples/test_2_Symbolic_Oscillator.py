...
def UserFunction( x: tor.Tensor ) -> tor.Tensor: return tor.max( tor.tensor( [ 0.0 ] ), tor.sin( 2 * x ) )
...
Expressions: list[ str ] = [
    "y[k-1]/x2[k]",
    "uFunc( x1[k-1] )",
    "1/abs( 0.2*x1[k-1] + 0.5*cos( x1[k-2]*x2[k] ) - 0.2 )",
    "~/(x1[k-1]*x2[k-1])",
    "~/(x2[k-2]^2)",
    "~/cos( 0.2*x1[k-3]*x2[k-1] + sin( x2[k-2] + 0.5 ) - 0.1 )"
]
...
def test_nonlinearity_configuration() -> None:
  ...
  assert names == [ "id", "uFunc", "abs", "cos", "sin" ]
  assert len( names ) == len( set( names ) )

  for nl in NonLinearities: assert isinstance( nl.get_f()( tor.randn( 10 ) ), tor.Tensor )
...
def test_symbolic_oscillator() -> None:
  ...
  def System( y: tor.Tensor, x1: tor.Tensor, x2: tor.Tensor, W: tor.Tensor, theta: tor.Tensor, StartIdx: int, EndIdx: int ) -> tor.Tensor:
    for k in range( StartIdx, EndIdx ):
      num = ( theta[ 0 ] * y[ k - 1 ] / x2[ k ] +
                         theta[ 1 ] * UserFunction( x1[ k - 1 ] ) +
                         theta[ 2 ] / tor.abs( 0.2 * x1[ k - 1 ] + 0.5 * tor.cos( x1[ k - 2 ] * x2[ k ] ) - 0.2 ) )
      den = ( 1 + theta[ 3 ] * x1[ k - 1 ] * x2[ k - 1 ] +
                         theta[ 4 ] * x2[ k - 2 ]**2 +
                         theta[ 5 ] * tor.cos( 0.2 * x1[ k - 3 ] * x2[ k - 1 ] + tor.sin( x2[ k - 2 ] + 0.5 ) - 0.1 ) )
      y[ k ] = W[ k ] + num / den
    return y

  AdditionalInput: tor.Tensor = 0.2 * ( tor.rand( p ) - 0.5 )

  theta: list[ tor.Tensor ] = [
  ...
  for i in range( 3 ):
    StartIdx: int = ( i * ThirdBuffer ) if i > 0 else 3
  ...
  for i in range( 3 ):
    ...
    out: tor.Tensor = Model.Oscillate(
