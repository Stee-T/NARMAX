...
from . import Parser_0_3 as Parser
...
# ################################################################################# Create EvalStr #################################################################################
def ParsedReg2EvalStr( RegNode: Parser.ExprNode, InputVarName2Idx: dict[ str, int ], NonLinName2Idx: dict[ str, int ], OutputVarName: str = 'y' ) -> tuple[ str, bool ]:
  ...
  ### Inputs:
  - `RegNode`: (ExprNode) the parsed expression node
  ...
  '''
  is_AR: bool = False # Flag tracking if the current regressor contains the output variable making it auto-regressive
  CoeffVal: float = RegNode.Coeff if ( RegNode.Coeff is not None ) else 1.0

  # -------------------------------- A. Function name / constant coefficient handling
  FuncStr: str = ""
  if ( RegNode.FuncName is not None ):
    if ( RegNode.FuncName in [ "~/", "1/" ] ): FuncStr = RegNode.FuncName # Those exact names: fractional/denominator with no supplementary Non-lin
    elif ( ( len( RegNode.FuncName ) > 2 ) and ( RegNode.FuncName[ : 2 ] in [ "~/", "1/" ] ) ):
      FuncStr = RegNode.FuncName[ : 2 ] + f"NonLinList[{ NonLinName2Idx[ RegNode.FuncName[ 2 : ] ] }].get_f()" # look-up without, but prepend
    else: FuncStr = f"NonLinList[{ NonLinName2Idx[ RegNode.FuncName ] }].get_f()" # Non-fractional aka Numerator expression

  # -------------------------------- B. Subexpression handling
  if ( ( not RegNode.SubExpressions ) and ( RegNode.FuncName is None ) ):
    if ( RegNode.VarName is None ): s: str = f"{ CoeffVal }"
    else: # MA
      VarStr: str = "OutVec" if RegNode.VarName == OutputVarName else f"Data[{ InputVarName2Idx[ RegNode.VarName ] }]"
      if ( RegNode.VarName == OutputVarName ): is_AR = True # reserve name to allow user to have y as variable
      LagStr: str = "[k]" if RegNode.Lag is None else f"[k{ RegNode.Lag:+}]"
      ExpStr: str = f"**{ RegNode.Exponent }" if RegNode.Exponent is not None else ""

      CoeffStr: str = ""
      if ( CoeffVal == 1.0 ): CoeffStr = ""
      elif ( CoeffVal == -1.0 ): CoeffStr = "-"
      else: CoeffStr = f"{ CoeffVal }*"
      s = f"{ CoeffStr }{ VarStr }{ LagStr }{ ExpStr }"
    return f"({ s })", is_AR # R[1/2]

  else:
    ChildStrs: list[ str ] = []
    for child in RegNode.SubExpressions:
      c_str: str
      c_is_AR: bool
      c_str, c_is_AR = ParsedReg2EvalStr( child, InputVarName2Idx, NonLinName2Idx, OutputVarName )
      ChildStrs.append( c_str )
      if ( c_is_AR ): is_AR = True

    InnerExpr: str = ChildStrs[ 0 ] if ChildStrs else ""
    for i in range( len( RegNode.Operators ) ):
      op: str = RegNode.Operators[ i ]
      if ( op == '^' ): op = '**'
      InnerExpr += f" { op } { ChildStrs[ i + 1 ] }"

    s = f"{ FuncStr }({ InnerExpr })" if RegNode.FuncName is not None else f"({ InnerExpr })"

    if ( CoeffVal == 1.0 ): CoeffStr = ""
    elif ( CoeffVal == -1.0 ): CoeffStr = "-"
    else: CoeffStr = f"{ CoeffVal }*"

    return ( f"({ CoeffStr }{ s })", is_AR ) # R[2/2]


############################################################################### Verify the Expression ##############################################################################
def VerifyParsedReg( Regressor: Parser.ExprNode, NonLinNames: list[ str ], ModelVarNames: list[ str ], OutputVarName: str = 'y' ) -> None:
  '''Verifies that:
  ...
  ### Inputs:
  - `Regressor`: (ExprNode) containing the expression AST (replaces ParsedReg)
  ...
  if ( Regressor.FuncName is not None ):
    ...
    if ( ( CurrentName.startswith( '~/' ) ) or ( CurrentName.startswith( '1/' ) ) ): CurrentName = CurrentName[ 2 : ]  # fractions are never stored as separate functions
  ...
  if ( Regressor.VarName is not None ):
    if ( Regressor.VarName not in ModelVarNames ):
      raise ValueError( f"Variable '{ Regressor.VarName }' is not declared in ModelVarNames list[str]" )
    if ( Regressor.VarName == OutputVarName ):
      if ( Regressor.Lag is None ):
        raise ValueError( f"No lag for output variable '{ OutputVarName }' is not supported (non-causality)" )
      if ( Regressor.Lag >= 0 ):
        raise ValueError( f"Positive lag for output variable '{ OutputVarName }' is not supported (non-causality)" )

  for subexp in Regressor.SubExpressions: VerifyParsedReg( subexp, NonLinNames, ModelVarNames, OutputVarName )
...
def Make_BufferStartSystem( Expr: str ) -> str:
  ...
  '''
  Output: str = re.sub( r'Data\[(\d+)\]\[k(([-+])(\d+))?\]', # Regexp recognizing Data = Input regressors
                  lambda match: f"Toggle(0, { int( match.group( 1 ) ) }, k{ match.group( 2 ) if match.group( 2 ) is not None else '' }, Data)", Expr )
  Output = re.sub( r'OutVec\[k(([-+])(\d+))?\]', # Regexp recognizing OutVec = Output regressors
                  lambda match: f"Toggle(1, None, k{ match.group( 1 ) if match.group( 1 ) is not None else '' }, None)", Output ) # Overwrite Output
  return Output


################################################################################ Make System Lambdas ###############################################################################
def Make_SystemLambdas( ModelVarNames: list[ str ], OutputVarName: str, NonLinName2Idx: dict[ str, int ], InputVarName2Idx: dict[ str, int ], RegStrList: list[ Parser.ExprNode ] ) -> tuple[ Optional[ Callable ], Optional[ Callable ], Callable, Callable ]:
  '''Creates lambda functions for system evaluation from parsed expression AST nodes.
  ...
  - `NonLinName2Idx`: (dict) mapping from non-linearity name to index
  - `InputVarName2Idx`: (dict) mapping from input variable name to index in Data
  - `RegStrList`: (list of ExprNode) parsed expression AST nodes (replaces list of ParsedReg)
  ...
  '''
  def _has_denom_prefix( node: Parser.ExprNode ) -> bool:
    if ( node.FuncName and ( node.FuncName[ : 2 ] == "~/" ) ): return True # R[1/3]
    for sub in node.SubExpressions:
      if ( _has_denom_prefix( sub ) ): return True # R[2/3]
    return False # R[3/3]

  # ----------------------------------------------------------------------------- 0. Verify Regressors -----------------------------------------------------------------------------
  for reg in RegStrList: VerifyParsedReg( reg, NonLinName2Idx.keys(), ModelVarNames, OutputVarName )
  ...
  for idx, reg in enumerate( RegStrList ):
    CurrentExpr: str; is_AR: bool # type declarations
    CurrentExpr, is_AR = ParsedReg2EvalStr( reg, InputVarName2Idx, NonLinName2Idx, OutputVarName )
    if ( _has_denom_prefix( reg ) ):
      if ( is_AR ): AR_DenExpr += f"theta[{ idx }]*{ CurrentExpr.replace( '~/', '' ).replace( '1/', '' ) } + "
      else:     MA_DenExpr += f"theta[{ idx }]*{ CurrentExpr.replace( '~/', '' ).replace( '1/', '' ) } + "
    elif ( reg.FuncName is None ):
  ...
  is_Rational: bool = ( MA_DenExpr != '' ) or ( AR_DenExpr != '' )

  DenExpr: str = ""
  # Optional Denominator Handling
  if ( is_Rational ): # Rational since at least one denom term exists
  ...
  Make_MA_System: Callable[ [ str ], str ] = lambda Expr: re.sub( r'Data\[(\d+)\]\[k(([-+])(\d+))?\]', # Regexp recognizing Data = Input regressors
                                                                  lambda match: f"Toggle({ int( match.group( 1 ) ) },{ match.group( 2 ) if match.group( 2 ) is not None else '0' },Data)",
                                                                  Expr
                                                                )

  SubSystem_MA_Num: Optional[ Callable ] = eval( f"lambda theta, Data, NonLinList, Toggle: { Make_MA_System( MA_NumExpr ) }" ) if MA_NumExpr else None
  SubSystem_MA_Den: Optional[ Callable ] = eval( f"lambda theta, Data, NonLinList, Toggle: { Make_MA_System( MA_DenExpr ) }" ) if MA_DenExpr else None

  # The final system has the form y[k] = ( MA_Num[k] + AR_Sys_Num( Data, k ) ) / ( MA_Den[k] + AR_Sys_Denom( Data, k) )
  # Bufferstart has no OutVec since accessed as class member
  SystemStart: str = f"lambda k, theta, Data, NonLinList, MA_Num, MA_Den, " # Standard form even if som MA are None
  if ( is_Rational ):
...
class SymbolicOscillator:
  ...
  def __init__( self, ModelVarNames: list[ str ], NonLinearities: list[ NL.NonLinearity ], ExprList: list[ str ],
    ...
    if ( not isinstance( theta, tor.Tensor ) ): raise ValueError( "theta must be of type 'torch.Tensor'" )

    if ( not isinstance( OutputVarName, str ) ): raise ValueError( "OutputVarName is not of type 'str'" )

    if ( not isinstance( dtype, tor.dtype ) ): raise ValueError( "dtype is not of type 'torch.dtype'" )
    ...
    self.SubSystem_MA_Num: Optional[ Callable ] = None # Non-recursive Numerator part (GPU)
    self.SubSystem_MA_Den: Optional[ Callable ] = None # Non-recursive Denominator part (GPU)
    self.System_BufferStart: Optional[ Callable ] = None # Full system for buffer start, calls AR_Toggle to dispatch between storage and input (CPU)
    self.System_Main: Optional[ Callable ] = None # Full system for main buffer (CPU)
    ...
    RegStrList: list[ Parser.ExprNode ] = [ Parser.ExpressionParser( expr ) for expr in ExprList ]
    ...
    def find_max_lags( node: Parser.ExprNode ) -> None:
      if ( ( node.VarName is not None ) and ( node.Lag is not None ) and ( node.Lag != 0 ) ):
        if ( node.VarName == OutputVarName ): self.MaxOutputLag = max( self.MaxOutputLag, abs( node.Lag ) )
        else:
          if ( node.Lag < 0 ): self.MaxInputLag = max( self.MaxInputLag, abs( node.Lag ) )
          elif ( node.Lag > 0 ): self.MaxPositiveInputLag = max( self.MaxPositiveInputLag, node.Lag )
      for sub in node.SubExpressions: find_max_lags( sub ) # recusrive call

    for reg in RegStrList: find_max_lags( reg )
  ...
  def Oscillate( self, Data: Sequence[ tor.Tensor ], theta: Optional[ tor.Tensor ] = None, DsData: Optional[ tor.Tensor ] = None ) -> tor.Tensor:
    ...
    if ( self.MaxInputLag > 0 ): # Don't trigger for systems being memoryless in the input terms (only x[k] terms)
      for inputIdx in range ( len( Data ) ): # Must iterate one by one since Data is an arbirtary iterable not necessarily a 2D Tensor
        if ( Data[ inputIdx ].ndim != 1 ): raise ValueError( f"Input Data must be 1D. The { inputIdx }-th input is not" )
        self.InputStorage[ inputIdx ] = Data[ inputIdx ][ -self.MaxInputLag : ].clone()
