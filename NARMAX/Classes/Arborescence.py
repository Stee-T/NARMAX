...
class Arborescence:
  ...
  def __init__( self, y: Optional[ tor.Tensor ] = None, # System Response / Data
                ...
                tolRoot: float = 0.001, tolRest: float = 0.001, MaxDepth: int = 5, # Arbo size influencers: rho 1 & 2 and max number of levels
                ...
                Verbose: bool = False, # rFOrLSR progess feedback
                MorphDict: Optional[ dict[ str, Any ] ] = None, U: Optional[ list[ int ] ] = None, # Morphing related stuff
    ...
    - `tolRoot`: (float) containing the maximum summed ERR threshold used for the root regression
    - `tolRest`: (float) containing the maximum summed ERR threshold used for all non-root nodes
    - `MaxDepth`: (int >= 0) containing the maximum number of levels to be iterated
    ...
    - `U`: (None or list[int >=0] ) containing the indices of all Dc columns allowed as candidate regressors
    ...
    if ( ( Dc is not None ) and ( not isinstance( Dc, tor.Tensor ) ) ): raise ValueError( "Dc must be None or a torch.Tensor" )
    ...
    if ( not isinstance( tolRoot, float ) ): raise ValueError( "tolRoot must be a float" )

    if ( not isinstance( tolRest, float ) ): raise ValueError( "tolRest must be a float" )

    if ( not isinstance( MaxDepth, int ) ): raise ValueError( "MaxDepth must be an int >= 0" )
    ...
    if ( not isinstance( Verbose, bool ) ): raise ValueError( "verbose must be a bool" )
    ...
    if ( ( U is not None ) and ( not isinstance( U, list ) or not all( isinstance( item, int ) for item in U ) ) ):
      raise ValueError( "U must be None or a list of ints >=0" )
    ...
    self.DsMeans: Optional[ tor.Tensor ] = None # Either stay None (no Ds) or updated below
    self.Dc: Optional[ tor.Tensor ] = Dc # columnwise centering and duplicate removal performed below
    self.DcMeans: Optional[ tor.Tensor ] = None # Overwritten below, when Dc's non-redundant size (nC) is known after filtering duplicate regressors
    ...
    self.tolRoot: float = tolRoot
    self.tolRest: float = tolRest
    ...
    if ( self.Ds is None ):
      ...
      self.DsMeans: tor.Tensor = tor.mean( self.Ds, axis = 0, keepdims = True ) # Store the means ( needed to revert the centering )
      self.Ds -= self.DsMeans # Columnwise centering
    ...
    if ( self.ValFunc is None ): # default to explained variance if no validation function is passed
      self.ValFunc = lambda theta, L, ERR, RegNames, ValidationData, DcFilterIdx = None: 1 - tor.sum( tor.tensor( ERR ) )
    ...
    if ( U is not None ):
      ...
      self.U: list[ int ] = U # Take unused index set passed by the user

    else: self.U: list[ int ] = [ j for j in range( self.nC ) ] # Nothing was passed, so assume entire dictionary (filtered of duplicates) can be used
    ...
    self.theta: Optional[ tor.Tensor ] = None # theta as None is used as verification if the validation function has been done, so no-touchy. Internally a tor.Tensor stored on CPU
    self.ERR: Optional[ NDArray[ np.float64 ] ] = None
    self.L: Optional[ NDArray[ self.INT_TYPE ] ] = None


  ################################################################################### Memory dump ##################################################################################
  def MemoryDump( self, nComputed: int ) -> None:
    '''
    Dumps the current Arborescence into a pickle file to load it later with load().

    ### Inputs:
    - `nComputed`: (int) number of nodes computed so far, used to restore the progress bar
    '''
    ...
    with open( self.FileName, 'wb' ) as file: # writes to or overwrites any existing file
      ...
      dill.dump( self.DsMeans, file );        ProgressCount.update()
  ...
  ################################################################################### Memory Load ##################################################################################
  def load( self, FileName: str, Print: bool = True ) -> None:
    ...
    old_state = self.__dict__.copy() # Shallow copy of references for rollback without duplicating tensor memory
    try:
      with open( FileName, 'rb' ) as file:
        ...
        self.DsMeans: Optional[ tor.Tensor ] = dill.load( file )
        ...
        self.tolRoot: float = dill.load( file )
        self.tolRest: float = dill.load( file )
        self.MaxDepth: int = dill.load( file )
        ...
        self.Verbose: bool = dill.load( file )
        self.MorphDict: Optional[ dict[ str, Any ] ] = dill.load( file )
        self.U: list[ int ] = dill.load( file )
        ...
        if ( Print ): # print some stats
          print( "\nTolerance: " + str( self.tolRest ), "\nArboDepth:", self.MaxDepth, "\nCurrent Dict-shape:", self.Dc.shape if ( self.Dc is not None ) else "N/A",
      ...
      raise RuntimeError( f"Deserialization failed. State rolled back. Cause: { e }" ) from e

  ##################################################################################### rFOrLSR ####################################################################################
  def rFOrLSR( self, y: tor.Tensor, Ds: Optional[ tor.Tensor ] = None, Dc: Optional[ tor.Tensor ] = None, U: Optional[ list[ int ] ] = None,
               tol: float = 0.001, MaxTerms: Union[ int, float ] = tor.inf, OutputAll: bool = False, LI: Optional[ Sequence[ int ] ] = None
             ) -> Union[ Tuple[ NDArray[ np.int64 ] ], Tuple[ NDArray[ np.int64 ], Any ], Tuple[ tor.Tensor, NDArray[ np.int64 ], NDArray[ np.float64 ] ] ]:
    '''
    ...
    - `U`: ( None or list[int > 0] ) containing the column numbers of Dc the rFOrLSR is allowed to use, None defaults to all
    ...
    if ( ( s > MaxTerms ) or ( OutputAll == False ) ): return ( tuple( [ np.array( L, dtype = self.INT_TYPE ) ] ) ) # R[3/4] only triggered if MaxTerms overwritten, L → int, since if self.Dc=None then L=[] is assuemd float
    else:
  ...
  def fit( self, FileName: Optional[ str ] = None, SaveFrequency: Union[ int, float ] = 0 ) -> Tuple[ tor.Tensor, NDArray[ np.int64 ], NDArray[ np.float64 ], dict[ str, Any ], tor.Tensor, NDArray[ np.str_ ] ]:
    '''Breadth-first Search Arborescent rFOrLSR (AOrLSR)

    ### Inputs:
    - `FileName`: (None or str) FileName to the pickle file
    - `SaveFrequency`: (int or float) frequency in minutes of saving the Arborescence content ( default: 0 for no saving)
  ...
  def validate( self ) -> Tuple[ tor.Tensor, NDArray[ np.int64 ], NDArray[ np.float64 ], dict[ str, Any ], tor.Tensor, NDArray[ np.str_ ] ]:
    ...
    for reg in tqdm.tqdm( self.LG.Data, desc = "Validating", unit = " Regressions" ): # and over each regressor sequence
      if ( len( reg ) == self.MinLen[ -1 ] ): # compute error metric only if candidate to best regression.
        ...
        if ( tuple( reg ) not in Processed ): # check that not somehow a permutation of already computed regression
          ...
          if ( Error < MinError ): MinError: float = Error; self.theta = theta.cpu(); self.L = reg.astype( np.int64 ); self.ERR = ERR # update best model
    ...
    if ( self.DsMeans is not None ): self.Ds += self.DsMeans # re-introduce means
    return ( self.get_Results() )


  ############################################################################ Regression Results getter ###########################################################################
  def get_Results( self ) -> Tuple[ tor.Tensor, NDArray[ np.int64 ], NDArray[ np.float64 ], dict[ str, Any ], tor.Tensor, NDArray[ np.str_ ] ]:
    ...
    return ( self.theta.to( Device ), self.L, self.ERR, self.MorphDict, self.Dc, self.DcNames )
  ...
  def set_ArboDepth( self, Depth: int ) -> None:
    ...
    ### Inputs:
    - `Depth`: (int) desired new maximum depth for the arborescence

    ### Raises:
    - `AssertionError`: if Depth is negative, already passed, or arbo is in its last level
    '''
  ...
  ################################################################################## Plot and Print ################################################################################
  def PlotAndPrint( self, ValData: dict[ str, Any ], PrintRegressors: bool = True, ValDataIdx: int = 0 ) -> Tuple[ Tuple[ Figure, Figure ], Tuple[ Axes, Axes ] ]:
    ...
    ### Inputs:
    - `ValData`: (dict) containing both data and model structure for the validation
    - `PrintRegressors`: (bool) whether to print the regressors to the console
    - `ValDataIdx`: (int) index of the validation data to use
    ...
    with plt.style.context( 'dark_background' ):
      ...
      Ax[ 0 ].plot( ValData[ "y" ][ ValDataIdx ].cpu(), "#00aaffff", marker = '.', markersize = 5 ) # force slightly lighter blue than default blue for compatibility with dark mode
    ...
    SortedERR: NDArray[ np.float64 ] = self.ERR[ Order ]; ChosenRegNames: NDArray[ np.str_ ] = ChosenRegNames[ Order ] # impose same order on all datastructures
    ...
    with plt.style.context( 'dark_background' ):
      ...
      yMax = max( [ i if i < np.inf else 0 for i in MAE ] )
      Ax2[ 1 ].set_ylim( [ 0, yMax * 1.3 if yMax > 0 else 1.0 ] )
