...
def DMFunc( yO: tor.Tensor, Xl: tor.Tensor, ksi: tor.Tensor, PA: tor.Tensor, f: Callable ) -> tor.Tensor:
  '''Correlation cost function for the dictionary morphing optimizer.
  Assumes that yO is already centered and normed.

  ### Inputs:
  - `yO`: ((p,1)-Tensor) orthogonalized system output
  - `Xl`: ((p,r)-Tensor) regressor matrix for the morphing subspace
  - `ksi`: ((r,1)-Tensor) linear combination coefficients
  - `PA`: ((p,p)-Tensor) orthogonal projection matrix
  - `f`: (callable) non-linearity function
...
# ************************************************************************************************ Optimizer: Gradient ********************************************************************************
def DMGrad( yO: tor.Tensor, Xl: tor.Tensor, ksi: tor.Tensor, PA: tor.Tensor, f: Callable, fPrime: Callable ) -> tor.Tensor:
  '''Gradient of the dictionary morphing cost function.
  ...
  - `PA`: ((p,p)-Tensor) orthogonal projection matrix
  - `f`: (callable) non-linearity function
  - `fPrime`: (callable) first derivative of the non-linearity function
...
# ************************************************************************************************ Optimizer: Hessian ********************************************************************************
def DMHessian( yO: tor.Tensor, Xl: tor.Tensor, ksi: tor.Tensor, PA: tor.Tensor, f: Callable, fPrime: Callable, fSecond: Callable ) -> tor.Tensor:
  ...
  - `f`: (callable) non-linearity function
  - `fPrime`: (callable) first derivative of the non-linearity function
  - `fSecond`: (callable) second derivative of the non-linearity function

  ### Returns:
  - (torch.Tensor) Hessian matrix (r x r)
  '''
...
# ************************************************************************************************ Expression Parser ********************************************************************************
def Expressionparser( idx: int, T: list ) -> tuple[ int, int ]:
  '''Parses the morphing table to find the actual regressor index and function index.

  ### Inputs:
  - `idx`: (int) column index in the dictionary matrix
  - `T`: (list of tuples) morphing table containing non-linearity indices and element lists

  ### Returns:
  - (tuple) actual regressor index i0 and function index fs
  '''
  ...
  return ( T[ fs ][ 1 ][ idx - CumSum ], fs ) # return actual index i0 and the function index fs



# ************************************************************************************************ Is Morphable ********************************************************************************
def IsMorphable( f: Callable, x0: tor.Tensor, order: int ) -> bool:
  ...
  - `order`: ( int ) indicating the morphing order, used to return False if>0 as any function is non-zero order morphable # R[1/4]
...
def GenVSGen( f: Callable, U: list, i0: int, Dc: tor.Tensor, order: int, Psi: Optional[ tor.Tensor ], Psi_n: tor.Tensor, y: tor.Tensor, m: tor.Tensor ) -> tuple[ list, list ]:
  '''
...
def InfOPT( y: tor.Tensor, Xl: tor.Tensor, ksiS: tor.Tensor, Ds: Optional[ tor.Tensor ], Dc: tor.Tensor, PA: tor.Tensor, f: Callable, fPrime: Callable, fSecond: Callable, Reps: int, A_T: list, Psi: Optional[ tor.Tensor ], Psi_n: tor.Tensor, W_T: list, L: list ) -> tor.Tensor:
  '''
  ...
  ### Inputs:
  - `y`: ((p,)-Tensor) system output
  - `Xl`: ((p,r)-Tensor) regressor matrix for the morphing subspace
  - `ksiS`: ((r,)-Tensor) initial linear combination coefficients from GenVSGen
  - `Ds`: (None or (p,nS)-Tensor) imposed regressor matrix
  - `Dc`: ((p,nC)-Tensor) candidate regressor dictionary
  - `PA`: ((p,p)-Tensor) orthogonal projection matrix
  - `f`: (callable) non-linearity function
  - `fPrime`: (callable) first derivative of the non-linearity function
  - `fSecond`: (callable) second derivative of the non-linearity function
  - `Reps`: (int) number of random retriggering repetitions
  - `A_T`: (list) triangular matrix copy for regression coefficient computation
  - `Psi`: (None or (p,s)-Tensor) unnormed orthogonal regressor matrix
  - `Psi_n`: ((p,s)-Tensor) normed orthogonal regressor matrix
  - `W_T`: (list) orthogonal regression coefficients
  - `L`: (list) selected regressor indices

  ### Returns:
  - (torch.Tensor) optimized linear combination coefficients ksi
  '''
...
# ************************************************************************************************ Morphing function ********************************************************************************
def DictionaryMorpher( U: list, ell: int, Psi: Optional[ tor.Tensor ], Psi_n: tor.Tensor, y: tor.Tensor, A_T: list, W_T: list, L: list, Ds: Optional[ tor.Tensor ], Dc: tor.Tensor, MDict: dict ) -> Optional[ Tuple[ tuple, tor.Tensor, str ] ]:
  ...
  ### Inputs:
  - `U`: (list of int) indices of candidate regressors allowed for selection
  - `ell`: (int) index of the selected regressor in U
  - `Psi`: (None or (p,s)-Tensor) unnormed orthogonal regressor matrix
  - `Psi_n`: ((p,s)-Tensor) normed orthogonal regressor matrix
  - `y`: ((p,)-Tensor) system output
  - `A_T`: (list) triangular matrix for coefficient computation
  - `W_T`: (list) orthogonal regression coefficients
  - `L`: (list) selected regressor indices
  - `Ds`: (None or (p,nS)-Tensor) imposed regressor matrix
  - `Dc`: ((p,nC)-Tensor) candidate regressor dictionary
  - `MDict`: (dict) morphing dictionary containing fPtrs, f1Ptrs, f2Ptrs, T, Order, DcMeans, DcNames, fStrs

  ### Returns:
  - (None or tuple) if morphable: (MorphData, Reg, Regstr), otherwise None
  '''
  ...
  if ( not Morphable ): return ( None ) # R[1/2]
  ...
  return ( MorphData, Reg, Regstr ) # R[2/2]
