...
def InputCheck( x: tor.Tensor, W: Optional[ tor.Tensor ], Print: bool ) -> None:
  '''Validate common input parameters for system generator functions.

  Parameters
  ----------
  x : torch.Tensor
      1D input signal tensor.
  W : torch.Tensor or None
      Optional additive noise tensor (same length as x).
  Print : bool
      Whether the system equation should be printed.

  Raises
  ------
  ValueError
      If any input has an invalid type or dimension.
  '''
...
def RationalNARMAX( x: tor.Tensor, W: Optional[ tor.Tensor ] = None, Print: bool = True ) -> Tuple[ tor.Tensor, tor.Tensor, Optional[ tor.Tensor ] ]:
  '''y[k] = \n(2x[k] -0.4|x[k-1]^3| + x[k-2]*x[k-1]^2 + 0.5tan(x[k-3]) /\n(0.5cos(y[k-1]) + 2y[k-2]^2)
