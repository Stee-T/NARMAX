...
def Lagger( Data: Sequence[ tor.Tensor ], Lags: Sequence[ Union[ int, Sequence[ int ] ] ], RegNames: Optional[ Union[ Sequence[ str ], NDArray[ np.str_ ] ]] = None,
  ...
  ### Input:
  - `Data`: ( sequence of (p,)-shaped Tensor ) ) containing the input
  - `Lags`: ( sequence of  (ints or sequences of int ) ) ) containing the maximum lags of each terms (int) or the lags of each regressor (sequence of ints)
  - `RegNames`: ( nupy array orsequence of strings ) containing the regressor names
...
def Expander( Data: tor.Tensor, RegNames: Union[ Sequence[ str ], NDArray[ np.str_ ] ], ExpansionOrder: int, IteractionOnly: bool = False
  ...
  - `RegNames`: ((nC,)-shaped numpy array or Sequence) containing the column names
...
def NonLinearizer( y: Optional[ tor.Tensor ], Data: tor.Tensor, RegNames: Union[ Sequence[ str ], NDArray[ np.str_ ] ],
  ...
  - `RegNames`: ((nc,)-dimensional ndarray / Sequence of str) containing the regressor names
...
#   -`RegCoeffs`: ( sequence of len=nDerivatives ) containing the regularization coefficients weighting the derivative by scaling that vector segment.
