...
#################################################################################### Lagger CTor ###################################################################################
def Lagger( Data: Sequence[ tor.Tensor ], Lags: Sequence[ Union[ int, Sequence[ int ] ] ], RegNames: Optional[ Sequence[ str ] ] = None,
  ...
  - `Lags`: ( iterable of  (ints or sequences ) ) ) containing the maximum lags of each terms (int) or the lags of each regressor (iterable of ints)
...
# ############################################################################### Monomial Expansion ###############################################################################
def Expander( Data: tor.Tensor, RegNames: Sequence[ str ], ExpansionOrder: int, IteractionOnly: bool = False ) -> ( tuple[ tor.Tensor, NDArray[ np.str_ ] ]):
  ...
  - `RegNames`: ((nC,) Sequence) containing the column names
...
def NonLinearizer( y: Optional[ tor.Tensor ], Data: tor.Tensor, RegNames: Sequence[ str ], Functions: Sequence[ NARMAX.NonLinearity ], MakeRational: Optional[ Sequence[ bool ] ] = None
  ...
  - `Functions`: (Sequence of NonLinearity objects) containing the functions to apply to the data
  - `MakeRational`: (Sequence of bools) containing a True for each function to be make rational
...
###################################################################################### Booler ######################################################################################
def Booler( Data: Union[ tor.Tensor, Sequence[ bool ] ], RegNames: Sequence[ str ],
            Operations: list[ Callable[ [ tor.Tensor, tor.Tensor ], tor.Tensor ] ] = [ tor.logical_and, tor.logical_xor, tor.logical_or ],
            OperationNames: Sequence[ str ] = [ "&&", "^", "||" ], AllowNegation: bool = True ) -> tuple[ tor.Tensor, NDArray[ np.str_ ] ]:
  '''Applies the list of passed binary operations elementwise on the passed data and re-labels the columns to take that into account if negation is enabled.
    If no operations are passed and `AllowNegation` is False, no transformation is performed and a warning is emitted.

    ### Input:
    - `Data`: ( (p, n)-shaped torch.Tensor or sequence of p booleans) The original regressor matrix (or list of booleans) with n columns.  
    - `RegNames`: (sequence of n strings) Names of the original regressors; length must equal n.  
    - `Operations`: (list of callables Tensor x Tensor → Tensor) Binary operations to apply between pairs of columns.  
    - `OperationNames`: (sequence of strings) Symbols to use when constructing new column names (must match Operations length).  
    - `AllowNegation`: (bool) If True, also include the element-wise logical NOT of each original column before combining.

    ### Output:
    - `Data`: ((p, m)-shaped torch.Tensor) The transformed regressor matrix with all requested operations, with constant columns (all-zeros or all-ones) removed.  
      Here m = number of retained combinations after filtering.  
    - `RegNames`: ((m,)-shaped np.ndarray[str]) The new column names corresponding to the retained columns in Data.
    '''
  ...
  if ( isinstance( Data, list | tuple ) ): DataList: list[ tor.Tensor ] = [ tor.column_stack( Data ) ] # make single tensor
  else:                                    DataList: list[ tor.Tensor ] = [ Data ] # already single tensor
  ...
  DataList: tor.Tensor = tor.column_stack( DataList )
  Sum: int = tor.sum( DataList, axis = 0 )
  IdxToKeep: tor.Tensor = tor.logical_not( tor.logical_or( Sum == DataList.shape[ 0 ], Sum == 0 ) )
