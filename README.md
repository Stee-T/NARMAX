...
**Note:** The library currently only implements the arborescence part (see below) and is thus not finished, missing the dictionary morphing part. This means that currently only static regressors can be fitted. Thus, the dictionary terms need to be pre-defined and are not adapted to the system by the regression.
...
$y = \text{sgn}(x)(1-\frac{1}{1+|x|A})$ with $A≔\sum_{j\in J}\theta _j |x|^j$ and $J\subseteq \mathbb{N}$
...
### Dictionary Morphing
**→ COMMING SOON**  
Essentially, an extension of the framework where the passed regressors are adapted to the system in real time by the rFOrLSR under very mild conditions. To illustrate, the user can pass any elementwise function $f_⦿$ and the arborescence will morph it by adding scaling coefficients $\xi_j$ and arbitrary linear combinations of dictionary terms as arguments. The morphing is thus of the form: 

$$f_⦿\left(\underline{\chi}_0\right) \rightarrow f_⦿\left(\sum_{j=0}^r \xi_j \underline{\chi}_j\right)$$
...
$\varphi(\left\|x_0 - c_0\right\|) → \varphi\left(\left\| \sum_{j=0}^r (a_j x_j-b_jc_j) \right\| \right)$ with scaling factors $\xi_j = a_j$ and $\xi_j = b_j$,  $\underline{c}_j = \underline{1}$
...
$M \underline{x}=\underline{y} \Leftrightarrow\left[\begin{array}{ccc}m_{1,1} & \cdots & m_{1,5} \\ \vdots & \ddots & \vdots \\ m_{p, 1} & \cdots & m_{p, 5}\end{array}\right]\left[\begin{array}{c}x_1 \\ \vdots \\ x_5\end{array}\right]=\left[\begin{array}{c}y_1 \\ \vdots \\ y_p\end{array}\right]$

The AOrLSR with a $1 \%$ error tolerance could chose the regressor (here columns of $M$) index-set $\mathcal{L}=\{1,3\}$ and $\underline{\hat{\theta}} \in \mathbb{R}^{|\mathcal{L}|=2}$, such that $\underline{x}[\mathcal{L}]:=\hat{\underline{\theta}}\iff \underline{x}=\left[\hat{\theta}_1, 0, \hat{\theta}_2, 0,0\right]^T$, while a $0.1 \%$ tolerance could yield $\mathcal{L}=\{1,3,4\}$ and $\underline{\hat{\theta}} \in \mathbb{R}^{|\mathcal{L}|=3}$, thus $\underline{x}=\left[\hat{\theta}_1, 0, \hat{\theta}_2, \hat{\theta}_3, 0\right]^T$.

The linear equation system is thus reduced to the most relevant  subset of $M$'s columns and $\underline{x}$ entries to describe the RHS $\underline{y}$: Thus, $\left[\underline{m}_j\right]_{j \in \mathcal{L}} \underline{\hat{\theta}}=\underline{y}$ is solved via a QR-like decomposition.
