...
# Example 1
Covers the basics of the library and demonstrates how to use the most common singnal-constructors and the arborescence.

**Fitted system**: $y\[k\] = 0.2x\[k\] + 0.3x^3\[k-1\] + 0.7|x\[k-2\]x^2\[k-1\]| +0.5e^{x\[k-3\]x\[k-2\]} - \cos(y\[k-1\]x\[k-2\]) -0.4|x\[k-1\]y^2\[k-2\]| - y^3\[k-3\]$

# Example 2
Is a short demonstration of how to fit rational models by linearizing the denominator regressors.
...
# Example 4
Demonstrates how to generate embedded expansions (advanced linearization and order determination) and how to create a custom validation function.
