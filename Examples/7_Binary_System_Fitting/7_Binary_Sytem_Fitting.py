...
while ( 5 ): # 5 is the absolute truth, do while y contains no nan
  ...
  x1 = tor.randint( 0, 2, size = (p,), device = rFOrLSR.device, dtype = tor.bool )
  x2 = tor.randint( 0, 2, size = (p,), device = rFOrLSR.device, dtype = tor.bool )
  x3 = tor.randint( 0, 2, size = (p,), device = rFOrLSR.device, dtype = tor.bool )
  x4 = tor.randint( 0, 2, size = (p,), device = rFOrLSR.device, dtype = tor.bool )

  x1, x2, x3, x4, y, W = rFOrLSR.Test.Binary_MISO_System( x1, x2, x3, x4, W, Print = False )
  if ( not tor.isnan( tor.sum( y ) ) ): break

VarNames = [ "x1", "x2", "x3", "x4" ]
Lags = [ 2, 2, 2, 2 ]
y, RegMat, RegNames = rFOrLSR.CTors.Lagger( [ x1, x2, x3, x4, y ], Lags + [ 0 ], VarNames + [ "y" ] )
RegMat, RegNames = rFOrLSR.CTors.Booler( RegMat, RegNames )
...
for i in range( 5 ): # Fill the validation dict's data entry with randomly generated validation data
  x1_val = tor.randint( 0, 2, size = ( int( p / 2 ), ), device = rFOrLSR.device, dtype = tor.bool )
  x2_val = tor.randint( 0, 2, size = ( int( p / 2 ), ), device = rFOrLSR.device, dtype = tor.bool )
  x3_val = tor.randint( 0, 2, size = ( int( p / 2 ), ), device = rFOrLSR.device, dtype = tor.bool )
  x4_val = tor.randint( 0, 2, size = ( int( p / 2 ), ), device = rFOrLSR.device, dtype = tor.bool )

  _, _, _, _, y_val, W = rFOrLSR.Test.Binary_MISO_System( x1_val, x2_val, x3_val, x4_val, W, Print = False )
...
def Bool_MAE( theta, L, ERR, RegNames, ValDic,  DcFilterIdx = None ): # The arborescence imposes 6 arguments to passed validation functions
  ...
  for i in range( len( ValDic["Data"] ) ): # iterate over all passed Data tuples
    _, RegMat, RegNames = rFOrLSR.CTors.Lagger( ValDic["Data"][i], ValDic["Lags"], ValDic["InputVarNames"] )
    RegMat, _ = rFOrLSR.CTors.Booler( RegMat, RegNames ) # create data
...
Arbo = rFOrLSR.Arborescence( y.to( tor.float64 ),
