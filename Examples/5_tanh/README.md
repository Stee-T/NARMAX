...
The arborescence compresses $\underline{L}$ to be in the smallest possible integer type (to save CPU-RAM, since millions of regressions are stored) but only 64-bit floats are currently valid python indices.
...
* **theta:** (1D torch.Tensor) containing the estimated regression coefficients.
