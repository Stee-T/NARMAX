...
def UserFunction( x: tor.Tensor ) -> tor.Tensor:
  return ( tor.max( tor.Tensor( [ 0 ] ), tor.sin( 2 * x ) ) ) # Some Weird oscillating ReLU
...
Expressions: list[ str ] = ["y[k-1]/x2[k]", "uFunc( x1[k-1] )", "1/abs( 0.2*x1[k-1] + 0.5*x1[k-2]*x2[k] - 0.2 )", # Num
