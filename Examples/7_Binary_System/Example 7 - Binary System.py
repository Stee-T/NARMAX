...
for i in range( len( theta ) ): print( theta[ i ].item(), RegNames[ L[ i ] ] )

with plt.style.context( 'dark_background' ):
  Fig, Ax = plt.subplots()
  Ax.plot( ( y - RegMat[ :, L.astype( np.int64 ) ].to( tor.get_default_dtype() ) @ theta ).cpu().numpy(), marker = '.', markersize = 6 )
  Ax.grid( which = 'both' )
  Ax.legend( [ "y - yHat" ] )
  Fig.tight_layout()
